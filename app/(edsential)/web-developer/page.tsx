...
const topics: Topic[] = [
  {
    ...
    description: "⚡️ **HTML (HyperText Markup Language)** คือภาษาหลักที่ใช้สร้างโครงสร้างและเนื้อหาของหน้าเว็บ เปรียบเสมือน 'กระดูก' ของเว็บไซต์ คลิปนี้จะสอนพื้นฐาน HTML5 และ CSS3 เบื้องต้นสำหรับผู้เริ่มต้น",
    videoId: "vH3i3pfPbBs", 
  ...
  },
  {
    ...
    title: "CSS",
    category: "Styling",
    description: "🎨 **CSS (Cascading Style Sheets)** คือภาษาที่ใช้ตกแต่งหน้าเว็บให้สวยงาม เปรียบเสมือน 'เสื้อผ้าและการแต่งหน้า' คลิปนี้จะเจาะลึก HTML5 & CSS3 Phase 1 เพื่อการตกแต่งเว็บไซต์อย่างมืออาชีพ",
    videoId: "HcInSUzhaUc", 
  ...
  },
  {
    ...
    description: "🧠 **JavaScript (JS)** คือหัวใจสำคัญของการทำเว็บให้มีลูกเล่น (Interactive) คลิปนี้คือคอร์สจัดเต็ม 8 ชั่วโมงจบในคลิปเดียว ครอบคลุมทุกพื้นฐานที่จำเป็น",
    videoId: "AbjY-ajKgSI", 
  ...
  },
  {
    ...
    description: "📦 **NPM (Node Package Manager)** คือเครื่องมือจัดการแพ็กเกจของ Node.js ที่ช่วยให้เราติดตั้ง Library ต่างๆ ได้ง่ายดาย คลิปนี้สอนการใช้ NPM และเข้าใจ package.json",
    videoId: "BQvVhzffVoE", 
  ...
  },
  {
    ...
    description: "💾 **Git & GitHub** คือระบบเก็บเวอร์ชันของโค้ดที่โปรแกรมเมอร์ทั่วโลกต้องใช้ คลิปนี้สอนตั้งแต่ติดตั้งจนถึงการ Push/Pull โค้ดขึ้น Cloud แบบจับมือทำ",
    videoId: "gqCzYy7mA3E", 
  ...
  },
  {
    ...
    description: "💨 **Tailwind CSS** คือ Utility-first Framework ที่มาแรงที่สุด ช่วยให้จัดหน้าเว็บได้ไวโดยไม่ต้องเขียน CSS เองเยอะ คลิปนี้ปูพื้นฐาน Tailwind เวอร์ชันใหม่",
    videoId: "wwY9cdTzgoE", 
    icon: <Layers className="w-6 h-6" />,
  ...
  },
  {
    ...
    description: "⚛️ **React** คือ Library ยอดนิยมจาก Facebook สำหรับสร้าง UI คลิปนี้สอน React เบื้องต้นจนถึงการนำไปใช้งานจริง เข้าใจ Concept Component-based",
    videoId: "ojT758BNvqg", 
    icon: <Code2 className="w-6 h-6" />,
  ...
  },
  {
    ...
    description: "🖥️ **Node.js** ช่วยให้เราเขียน JavaScript บนฝั่ง Server ได้ คลิปนี้ปูพื้นฐาน Node.js สำหรับการพัฒนาเว็บสมัยใหม่ เข้าใจการทำงานของ Backend",
    videoId: "QgDFVaIl-l8", 
    icon: <Server className="w-6 h-6" />,
  ...
  },
  {
    ...
    title: "PostgreSQL",
    category: "Database",
    description: "🗄️ **PostgreSQL** คือฐานข้อมูลระดับ Enterprise ที่ฟรีและทรงพลัง คลิปนี้สอนการใช้งานเบื้องต้นและการจัดการข้อมูลด้วย SQL",
    videoId: "o--rsYuxBhg", 
  ...
  },
  {
    ...
    description: "🔗 **RESTful API** คือมาตรฐานการส่งข้อมูลระหว่างหน้าเว็บ (Frontend) และหลังบ้าน (Backend) คลิปนี้สอนออกแบบและพัฒนา API ด้วย Express.js",
    videoId: "Zpsccboc_QE", 
    icon: <Globe className="w-6 h-6" />,
  ...
  },
  {
    ...
    description: "🚀 **Supabase** คือทางเลือก Open Source แทน Firebase ที่มี Database, Auth, Storage ครบจบในที่เดียว คลิปนี้แนะนำการเริ่มต้นใช้งานสำหรับมือใหม่",
    videoId: "uKTxcfFkwZg", 
    icon: <Database className="w-6 h-6" />,
...
const Modal: React.FC<ModalProps> = ({ isOpen, onClose, topic, isCompleted, onToggle }) => {
...
export default function WebDeveloperRoadmap() {
  ...
  const progressPercent = Math.round((completedIds.length / topics.length) * 100);
  ...
  return (
    <main className="min-h-screen bg-[#0B0D13] text-gray-300 selection:bg-purple-500/30">
      ...
      <div className="fixed top-0 left-0 right-0 z-40 bg-[#0B0D13]/90 backdrop-blur-md border-b border-white/5 px-4 py-3 shadow-lg">
        <div className="max-w-5xl mx-auto flex items-center gap-4">
            <div className="flex-1">
                ...
                <div className="h-2 w-full bg-gray-800 rounded-full overflow-hidden">
                    <div 
                        className={`h-full transition-all duration-500 ease-out ${progressPercent === 100 ? 'bg-linear-to-br from-green-400 to-green-600' : 'bg-linear-to-br from-purple-500 to-pink-500'}`}
      ...
      <div className="relative pt-28 pb-16 px-4 text-center overflow-hidden">
        ...
        <div className="relative z-10 max-w-4xl mx-auto">
          ...
          <p className="text-lg md:text-xl text-gray-400 max-w-2xl mx-auto leading-relaxed">
            เส้นทางการเรียนรู้สู่การเป็นนักพัฒนาเว็บมืออาชีพ เริ่มต้นจากศูนย์จนถึงการสร้างแอปพลิเคชันที่ใช้งานได้จริง เรียงลำดับความสำคัญไว้ให้คุณแล้ว
          </p>
        </div>
      </div>
      ...
      <div className="max-w-5xl mx-auto px-4 pb-32 mt-10">
        <div className="relative">
          ...
          {topics.map((topic, index) => {
            ...
            return (
              ...
              >
                ...
                <div className={`absolute left-8 md:left-1/2 -translate-x-[5px] md:-translate-x-1/2 w-4 h-4 rounded-full z-10 shadow-[0_0_15px_rgba(168,85,247,0.5)] transition-all duration-500 border-[3px]
                   ${isCompleted ? 'bg-green-500 border-green-500 scale-125' : 'bg-[#0B0D13] border-purple-500'}
                `}>
                    {!isCompleted && <div className="absolute inset-0 bg-purple-500 rounded-full animate-ping opacity-20"></div>}
                </div>
                ...
                <div className={`w-full md:w-1/2 pl-20 md:pl-0 ${isEven ? 'md:pr-12' : 'md:pl-12'}`}>
                  ...
                  >
                    ...
                    <div className={`absolute inset-0 bg-linear-to-br opacity-0 group-hover:opacity-100 transition-opacity duration-500
                        ${isCompleted ? 'from-green-500/5 to-transparent' : 'from-purple-500/5 to-transparent'}
                    `} />
                    ...
                    <div className="relative z-10">
                      ...
                      {/* Title */}
                      <h3 className={`text-2xl font-bold mb-2 transition-colors flex items-center gap-2
                        ${isCompleted ? 'text-gray-200' : 'text-white group-hover:text-purple-400'}
                      `}>
                        {topic.title}
                        {!isCompleted && <ChevronRight className="w-5 h-5 opacity-0 group-hover:opacity-100 -translate-x-2 group-hover:translate-x-0 transition-all text-purple-500" />}
                      </h3>
                      ...
                      <div className="flex items-center justify-between mt-4">
                        ...
                        <button 
                            ...
                            className={`z-20 p-2 rounded-full transition-all ${isCompleted ? 'text-green-500 bg-green-500/10' : 'text-gray-600 hover:bg-white/10 hover:text-gray-300'}`}
          ...
          <div className="relative flex justify-center mt-12 pl-8 md:pl-0">
             <div className={`px-6 py-3 rounded-full flex items-center gap-3 shadow-lg z-10 border transition-all duration-500
                ${progressPercent === 100 
                    ? 'bg-green-500/10 border-green-500/50 text-green-400 shadow-green-500/20' 
                    : 'bg-[#13151c] border-purple-500/30 text-purple-300'
                }
             `}>
                {progressPercent === 100 ? <Trophy className="w-5 h-5" /> : <CheckCircle2 className="w-5 h-5" />}
                <span className="font-semibold">{progressPercent === 100 ? "Ready to Work!" : "พร้อมทำงานจริง!"}</span>
             </div>
          </div>
      ...
      <Modal 
        ...
        isCompleted={selectedTopic ? completedIds.includes(selectedTopic.id) : false}
